...
> Last updated: 2026-05-12 | Current build: v72s
...
| Build | Key changes |
|-------|-------------|
| v72s | **Found the actual root cause of the navigation flash: redirect-skip.** Cloudflare Pages canonicalizes URLs by redirecting `/admin-foo.html` → `/admin-foo` (clean URL). Cross-document view transitions are explicitly skipped by browsers when a navigation involves a redirect — silently, no error. So every sidebar click was: click `admin-settings.html` link → 301 redirect → `admin-settings` → browser skips transition → white flash. v72q/r had configured everything correctly; the transition just never fired. **Fix:** stripped `.html` from all 22 internal admin links (10 in the sidebar partial + 12 scattered across pages) so navigations go straight to the canonical URL with no redirect hop. Also bumped `SIDEBAR_VERSION` to 2 in `admin-sidebar-inject.js` so existing browsers don't serve the old cached sidebar with the `.html` links. **Note on diagnosis:** three earlier builds (v72n partial, v72q view transitions, v72r body-visibility) all "should have" worked according to the docs, but kept missing this. The console diagnostic confirmed the API was set up correctly — the redirect was the silent skipper. Lesson: when something "should work but doesn't," check the navigation chain itself, not just the API |
| v72r | (Superseded by v72s) admin-gate skips body-hide on subsequent session loads; admin-fetch awaits Clerk-ready before /api/ calls. The body-hide change is still good defense-in-depth but wasn't the root cause |
| v72q | (Superseded by v72s) View transitions CSS in admin-chrome.css. Correctly configured but never fired due to the .html redirect chain |
