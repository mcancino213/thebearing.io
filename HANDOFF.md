...
> Last updated: 2026-05-14 | Current build: v73x
...
| Build | Key changes |
|-------|-------------|
| v73x | **Stripe smoke test fixes from first end-to-end run.** Miguel ran the test, payment processed, booking moved to Confirmed, customer + admin emails arrived, but two visible issues. (1) **Persistent "Booking confirmed" hero card on /bookings**, replacing the 8-second toast that was too easy to miss. Renders inline at the top of the Confirmed section with full receipt details: property name, dates, room, deposit paid amount, booking reference. Dismissable via X button. Triggered only when user lands at /bookings?checkout=success — sets `window.__tbShowConfirmedHero=true`, picked up by `renderConfirmedHeroCard()` which is prepended to `renderConfirmedList()` output. The most recently paid booking (by depositPaidAt) is the one highlighted, so multi-confirmed users still see the "you just paid for this one" framing. (2) **Stripe webhook fallback for missing conversationId on booking.** The system message "Booking confirmed" wasn't appearing in the conversation thread. Investigation found the worker's `if (convId)` check at envoy.js:3605 was silently skipping when `booking.conversationId` was falsy. New behavior: falls back to `meta.conversationId` (which is set in Stripe Checkout session metadata at create-session time, so we always have it as a backup). If both are missing, logs an explicit error so we never have to guess about silent skips again. Also self-heals: if booking record was missing conversationId but metadata had it, writes the value back onto the booking record so future operations find the linkage. (3) **Better webhook diagnostics** \u2014 explicit log lines at every branch: "posted booking_confirmed system message to conversation X", "NO conversationId on booking", "booking references conversation X but that conversation does not exist in KV". Means next silent-skip is impossible. **Not a bug clarification:** the "3 emails" we kept describing in handoffs (customer + admin + partner) is actually 2 emails until per-property partnerEmail is wired in v73y \u2014 the partner notification call checks `if (partnerEmail not in adminRecipients)` and skips because the placeholder `admin@thebearing.io` IS the admin baseline. By design, no duplicate to same address. Once partner emails are real per-property addresses, the 3rd email will start firing |
