...
> Last updated: 2026-05-14 | Current build: v73ad
...
| Build | Key changes |
|-------|-------------|
| v73ad | **Styled offer + booking-confirmed cards on admin and partner conversations.** Previously these system messages rendered as plain text bubbles (just `m.text` dumped into a regular bubble) which looked janky against the rest of the chat. Customer side has had the nice cards since v73j/v73q. Now admin and partner show parity \u2014 same cream-on-terracotta offer card with "Offer sent to guest" eyebrow + "View offer \u2192" CTA, same moss-green "Booking confirmed" card with check icon. CSS uses `!important` to override the role-* color gradients (admin's dark bubble, partner's terracotta bubble) so cards always have the same document-like treatment regardless of who sent them. **New shared asset `assets/offer-detail-modal.js`** exposes `window.TBOfferDetail.open(offerId, {mode:'admin'|'partner'})`. Reads `/api/offer?id=X` and renders read-only modal: status badge (Awaiting reply / Accepted / Declined / Withdrawn / Superseded / Draft), trip details, total + deposit with balance due date if set, valid-until, inclusions checklist, exclusions, cancellation terms, partner notes. Same look as customer's modal minus the Accept/Decline/Request-changes actions. Mode just toggles the eyebrow text ("Offer sent to guest" for partner, "Offer details" for admin). Included on both `admin-conversations.html` and `pp-conversations.html`. Hover actions (emoji react, copy, quote-reply) skipped on these card-type messages \u2014 parity with customer side, system cards aren't regular messages. Conv-list preview text was already nice (the worker writes "\ud83d\udccb Offer ready" / "\u2713 Booking confirmed" preview when posting the system message) so that just works |
