...
> Last updated: 2026-05-14 | Current build: v73s
...
| Build | Key changes |
|-------|-------------|
| v73s | **admin-settings layout hotfix for v73r.** When adding the new "Restore booking dates from active offer" maintenance section, I removed an extra `</div>` along with the trailing wrapper I no longer needed, leaving v73r with +1 div imbalance (76 opens vs 75 closes). The new section's button rendered outside the Maintenance card body — floating in the page background instead of inside the rounded gray-bordered container, breaking visual symmetry with "Backfill offer cards" above it. **Self-correction:** the bug class is identical to v73m's orphan-opener — relying on str_replace's mechanical match without visually confirming the structural nesting at the join site. The new section had balanced opens/closes internally (1 separator self-closing, 2 paired open/close blocks) — but my str_replace deleted one wrapper-closing `</div>` from the original layout, dropping the total close count by 1. Two checks would have caught it: (1) running the `<div>/<\/div>` count delta before/after my change (the 5-line v73r→v73s diff between count outputs), (2) actually looking at the structural nesting in the file after edit, not just at my new lines in isolation. Fix: added one closing `</div>` between the new section's pre and the outer wrappers. Validated: 76/76 balanced |
