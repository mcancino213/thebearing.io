...
> Last updated: 2026-05-14 | Current build: v73u
...
| Build | Key changes |
|-------|-------------|
| v73u | **Five fixes from Miguel's v73t testing + a free improvement.** (1) **Decline now copies pendingChangeRequest values back to booking** \u2014 v73r/t had a bug where declining an offer reverted the booking to the OFFER's frozen dates instead of the customer's most recent ask. If guest sent a change request (Jun 15-20) then declined, booking would revert to offer dates (Jul 2-4), erasing what the guest actually wanted. Worker `action:'decline'` now: if pendingChangeRequest exists, copies arrival/departure/guests/cabin/notes onto the booking shape and clears the pendingChangeRequest. If no pcr existed, leaves booking dates as offer dates (better than empty). Either way, sets `declinedAt` + `lastDeclinedOfferId` flags so the partner sees a "previously declined offer" signal rather than a row indistinguishable from a fresh enquiry. (2) **Sand-colored row treatment + "Previously declined offer" pill on pp-bookings** for bookings with `declinedAt` set. CSS conventions: sand = historical context (this was once an offer); amber = current action needed (pending change request). Only shows in `status:'enquiry'` state and when no pendingChangeRequest is active (so it doesn't compete with the more-urgent amber treatment). (3) **Removed bullet/dot from pendingChangeRequest pill** on pp-bookings rows. Was using an animated `pcr-pill-dot` span; visual review found it cramped the pill. Pill now reads cleanly: "Change requested: Jun 15 \u2013 Jun 20, 2026 \u00b7 2 adults guests \u00b7 Deluxe King Suite". Keyframe + CSS class definitions retained but unused (no harm). (4) **Revise modal banner squish fix** \u2014 v73r/t inserted the "Was/Now" banner as a flex-child sibling of the modal title block + close button, all sharing a horizontal flex row, which squished the title into a narrow column. New insertion: walk up from `om-title` to find the modal-header container, insert banner AFTER it (as a block-level sibling between header and body). Banner gets explicit `marginLeft:28px; marginRight:28px; marginTop:18px` to match the modal's body padding. (5) **Property thumbnails on customer /bookings rows** \u2014 new `propertyThumbCache` (module-scope, page-session) populated by `prefetchPropertyThumbs(bookings)` which collects unique slugs, fetches `/api/property?slug=X` in parallel via `Promise.all`, extracts `data.photos.hero[0].url`. **Two-pass render** strategy: render runs immediately with cache hits (empty linen-bg div fallback for misses), prefetch resolves, render runs again to swap in images. Renders before AND after prefetch resolves \u2014 user doesn't wait for property data, but images populate as soon as available. `thumbHtml(b)` helper returns `<img class="bk-row-thumb" src="..." loading="lazy">` when cached, empty div otherwise. Three call sites in bookings.html (enquiries/offers/confirmed sections) updated via batch replace; cancelled section already used `bk-row-thumb` empty divs and is left as-is (cancelled rows are historical, less need for visual identity). (BONUS) **Distinct email subject/body when guest message stores pendingChangeRequest.** Was: `"Reply from {guest} \u2014 {prop}"` with generic body. Now: `"Change request from {guest} \u2014 {prop}"` with structured body showing Was/Now diffs and an explicit "Action: open the booking, click Revise offer" line. Worker tracks via hoisted `storedChangeRequest` flag, set inside the updateEnquiry branch when pendingChangeRequest is written. Helps partner triage their inbox; change requests are categorically more urgent than chatty replies. **API shape gotcha caught during build:** initial v73u code read `data.property` from `/api/property?slug=X` response, but the actual shape is `{slug, data, exists}` where the property JSON lives at `data.data`. Fixed before shipping; thumbs now resolve correctly |
