...
> Last updated: 2026-05-13 | Current build: v73d
...
| Build | Key changes |
|-------|-------------|
| v73d | **Three fixes addressing Miguel's "I'm signed in but it says I'm not" report + the empty `From` block on Gypsy's booking card.** (1) **`openEnquiry()` now auth-gates the same way `tbAuthEnquire()` does.** Root cause of the sign-in error: the booking card's "Send an enquiry first" button (line 1167) and the mobile book-bar Enquire button (line 1145) both called `openEnquiry()` — a non-auth-gated shortcut that just opened the overlay directly. Other Enquire buttons used `tbAuthEnquire()` which DOES gate auth. Users could click these two specific buttons, fill the form, hit submit, and only THEN discover they weren't authenticated. `openEnquiry()` now wraps `tbAuth.requireUser()` like the others. (2) **Robust async Clerk-user resolution in `submitEnquiryAndOpenChannel`.** Previously the user-resolution was synchronous: read `window.Clerk.user` or `tbAuth.getUser()`, fail immediately if both null. But Clerk-via-cookie hydration can lag the initial page load. New approach: `resolveUserOrFail()` first tries synchronously, and on null, calls `tbAuth.loadClerk()` to force hydration before checking again. If still null, opens the Clerk sign-in modal directly (instead of telling the user to "click Enquire again to sign in") and recurses into the submission on success. This means even if a user somehow gets to the submit step without auth, they get a working sign-in flow rather than a dead-end error. (3) **`From` price block hides when property has no price.** Gypsy's KV record has no rooms or no first-room price → `dbc-price` and `dbc-pp` rendered empty, leaving "FROM" as an orphan label with blank space below it. `renderProperty()` now hides the `.dbc-price-from` label, the price/pp spans, and the first divider when no price is set. Book / Enquire buttons remain — pricing gets quoted by the property in their offer (which is the right model anyway given how the offer flow is designed). To populate it properly: set a room with price on the property in admin-property-editor |
