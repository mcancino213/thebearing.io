...
> Last updated: 2026-05-13 | Current build: v73e
...
| Build | Key changes |
|-------|-------------|
| v73e | **Three fixes: bulletproof sign-out + graceful no-price fallback + new price_indicator field.** (1) **Sign-out actually signs you out now.** Old code in `assets/account-sidebar.js` silently did nothing if `window.Clerk` wasn't loaded at click time (early click, slow network). It also used `window.location.href = '/'` which can be cached. Now: shows "Signing out…" feedback immediately; waits up to 4s for Clerk to load; calls `Clerk.signOut()` properly; clears all sessionStorage; redirects with `location.replace('/?signedout=' + Date.now())` so the previous page isn't in history and there's a cache-busting query string. Even if Clerk fails to load entirely, the cache is wiped and the redirect still fires — the user is no longer stuck. (2) **No-price properties get a graceful "On request" fallback instead of hiding everything.** v73d hid the whole `FROM` block when a property had no `rooms[].price` — that made the booking card feel empty and was unclear to the user (Miguel asked: "what is supposed to be in this 'from' area?"). Now: when no nightly price exists, the card shows "Starting from $X–Y / night · varies by season" if a `price_indicator` is set, or a softer "On request — Personalised quote in your offer · varies by season" fallback. Either way the user sees something meaningful, not an empty label. (3) **New `price_indicator` field on properties** — free-text seasonal pricing guidance set in admin-property-editor Section 0. Optional. Lets partners set something like "$500–1,200 / night · varies by season" or "$8,000 per 7-night charter" without committing to a single fixed price. Shown to members on the property page booking sidebar so they don't enquire flying blind. **Note on "dates are gone":** Miguel's screenshot showed two faint horizontal lines under "FROM" that he initially read as date inputs — they're CSS dividers, the booking card has never had inline date pickers. Dates are collected inside the enquiry overlay form (Step 1) and the booking checkout modal. The price_indicator + fallback together fill the empty space that prompted the confusion. **What's deferred:** Full seasonal pricing matrix (High/Low/Shoulder rates with month ranges) is out of scope — a v74+ decision when there's enough property volume to justify the UI |
