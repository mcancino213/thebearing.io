...
> Last updated: 2026-05-13 | Current build: v73f
...
| Build | Key changes |
|-------|-------------|
| v73f | **Three fixes unblocking the offer-flow smoke test.** (1) **Sign-in modal centering + Loading placeholder.** The Clerk widget mounts a ~440px-wide card inside our `#tb-clerk-mount` container; the modal sheet had `max-width:480px` which was too tight, causing the widget to anchor left while the header stayed full-width — and the "Loading sign-in…" placeholder text stayed visible alongside the widget because Clerk's mountSignIn appends rather than replaces. Bumped sheet max-width to 560px, removed the conflicting flex-center on the mount container, and explicitly removed the loading placeholder element right before calling mountSignIn. (2) **Sign-in resumes the enquiry flow instead of dropping you on the home page.** Two underlying bugs. First, the `stayInPlacePaths` whitelist that decides "where to send the user after sign-in" only matched legacy `.html` URLs (`/property.html`) — but Cloudflare Pages serves clean URLs (`/property`), so the check failed and afterSignInUrl became `/my-account.html`. Added clean URLs to the whitelist. Second, even with `stayInPlace=true` the user would land back on the property page with the enquiry overlay closed and the click context lost — because Clerk's OAuth flow is a full-page redirect, not just a modal close. New: `tbAuthEnquire()` and `openEnquiry()` stash a `tb_pending_action` key in sessionStorage before triggering sign-in. On page load, `resumePendingAction()` IIFE checks for the key, waits up to 8s for Clerk to confirm a real user, then auto-opens the enquiry overlay. Guards: 10-min expiry, slug-match (won't trigger Gypsy enquiry on Singita page if user navigated mid-flow). (3) **Partner portal cross-tenant testing via `?as=X` URL param.** Pre-v73f the partner portal hardcoded `PP_SLUG = 'nour-el-nil'` everywhere — there was no way to act as a different partner. So even with a Gypsy enquiry sitting in KV, the only partner view available was Nour El Nil. Now: `/pp-bookings?as=gypsy-by-mekong-kingdoms` views as Gypsy. Same for pp-conversations and sidebar badges. `pp-sidebar-badges.js` automatically rewrites every `.sb-item` sidebar link to carry the `?as=` param so nav doesn't lose context. Visible amber banner at top of page shows "Viewing partner portal as <slug>" with a "Switch back to default" link. No real auth — anyone can use it. Real multi-tenant partner auth is still on the deferred list. **Smoke test now:** `/pp-bookings?as=gypsy-by-mekong-kingdoms` should show Miguel's Mekong Kingdoms enquiry with a Build Offer button |
