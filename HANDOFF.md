...
> Last updated: 2026-05-15 | Current build: v73aj
...
| Build | Key changes |
|-------|-------------|
| v73aj | **Three follow-up fixes from Miguel's v73ai smoke test.** (1) **Sticky book-bar missing on Gypsy** \u2014 the dynamic `property.html` template only revealed the sticky "Enquire to book" bar at the top of viewport if `p.rooms[0].price` was set. Gypsy by Mekong Kingdoms has no per-room price (priced via custom offer, same as most luxury/experiential bookings) so the bar stayed `display:none` forever. Nour El Nil X (flagship hand-built page) had its own bar markup unconditionally visible. Fix: book-bar now always renders, with the same fallback the desktop booking card already used \u2014 if `price_indicator` field is set on the property record, show that; otherwise show "On request". The "FROM" eyebrow becomes "Starting" / "Pricing" to match. (2) **Partner sidebar badges silently never rendered when `?as=X` URL param was in use** \u2014 v73f added href rewriting on partner-portal sidebar links to preserve the `?as=X` admin-impersonation param across navigation. After rewrite, hrefs become `pp-conversations?as=gypsy-by-mekong-kingdoms` instead of `pp-conversations`. The badge update logic did exact-string matching against `'pp-conversations'` / `'pp-conversations.html'` so it never found a match and never set the badge. Same bug affected the Bookings badge. Fix: `pp-sidebar-badges.js` now matches by pathname only, stripping `?...` query and `.html` extension before comparison. Both Conversations and Bookings badges now render correctly for `?as=X` sessions. (3) **Build offer modal showed duplicate room input fields** \u2014 when the booking's `room` field had a value that didn't match any of the property's known room types (e.g. a Gypsy booking with `room: 'Luxury Room'` from a cross-property test), the v73ah custom dropdown auto-opened its inline custom-text input AND showed the value in the dropdown trigger, producing two visible "Luxury Room" fields stacked vertically. Fix: `setRoomDropdownValue` no longer auto-opens the custom input on non-matching prefill. Instead, the dropdown label shows the value with a small "(custom)" hint to indicate why it's not a standard room. If the partner wants to edit, they click the dropdown and select "Other / custom\u2026" \u2014 which now preserves the existing custom value as the starting text (was: blanked it). The custom input's `input` listener also syncs the dropdown trigger label as the partner types, so the visible state stays consistent |
