...
> Last updated: 2026-05-14 | Current build: v73r
...
| Build | Key changes |
|-------|-------------|
| v73r | **Fixed silent data-corruption bug where re-enquiry overwrote booking dates while offer stayed frozen.** Miguel caught this during Stripe smoke-testing: he sent himself an offer for Jul 2-4 ($1000/$150), then went to the property page and submitted a fresh "enquiry" with new dates (Jun 22-27). The v73i re-enquiry logic posted his new dates as a follow-up message to the existing conversation AND silently rewrote the booking record's arrival/departure fields. The OFFER kept its original dates frozen (correct), the BOOKING got the new dates (silent mutation, no flag). Result: customer sees offer with original dates, pp-bookings shows new dates, Revise modal prefills with old dates, no one sees the new dates anywhere visible to the partner except inside a conversation message body. Worst case: customer pays deposit assuming new dates, property confirms for old dates, mismatch discovered at check-in. Four-part fix:  (1) **Worker hardening** \u2014 `action:'message'` handler at envoy.js ~L1738 now checks if `conv.bookingRef`'s booking has `active_offer_id`. If yes, stores `pendingChangeRequest` object on the booking with requested fields + snapshot of previous values + requestedAt timestamp, but does NOT mutate booking dates. If no active offer, behaves as before (fresh-enquiry path unchanged). The mutation guard also blocks `conv.enquiry` shape rewrites since those flow into the partner's side panel. (2) **New worker endpoint** `PATCH /api/offer` with `{action:'decline', id, requesterEmail, reason?}`. Email-auth like cancel-enquiry. Only valid from `status:'sent'`. Sets offer.status='declined' + responded_at + declined_reason, resets booking.status to 'enquiry' (so partner sees Build offer again), clears active_offer_id and pendingChangeRequest, posts a `type:'offer_declined'` system message into the conversation, sends Resend email to admin. (3) **Partner UI surfacing** on pp-bookings: rows with `pendingChangeRequest` get an amber row treatment (3px stripe + bg tint), an animated "Change requested: Jun 22-27" pill in the dates column showing the requested values, and the Revise offer modal **prefills from the change request** (not the previous offer) and shows a sand-colored banner at the top explaining what changed ("Was: Jul 2-4 / Now: Jun 22-27"). Partner has full context now. (4) **Customer interstitial on /property** \u2014 before submitting an enquiry on a property where the customer has an active offer, intercept with a three-option modal: **View offer** (navigates to /conversations.html?id=X), **Request changes** (posts a structured change-request message into the conversation using the form values they just entered + updateEnquiry payload, then navigates), **Decline this offer** (confirms via window.confirm then calls PATCH /api/offer action:'decline', navigates to conversation). Plus a Cancel option ("Don't do anything"). The button has a hover-rich card style. Interstitial only fires when the GET /api/booking?email=X returns a booking with active_offer_id matching the current property slug; otherwise the normal enquiry flow proceeds. Fails-open on network error (allows enquiry through rather than blocking legit ones). (5) **Admin maintenance button** at admin-settings: "Restore booking dates from active offer". Walks `__bookings_index`, for each booking with `active_offer_id` copies offer.arrival/departure/nights/guests/room back to the booking record and clears any stale pendingChangeRequest. Idempotent (skips bookings where dates already match). Used once to repair TB-2026-1190 corruption from Miguel's smoke test. New endpoint: POST /api/booking/restore-from-offer (admin-gated). **Architecture note:** The interstitial gives customers explicit agency over their offer (View / Request changes / Decline) rather than the previous coercive state where the only way to escape an offer they didn't like was to cancel the entire enquiry or accept (which costs money). Decline action is also wireable into the existing offer modal in /conversations.html and /bookings.html as a follow-up if Miguel asks \u2014 deferred since the interstitial covers the main missed path. **Stripe SDK is still pinned at v17/`2025-04-30.basil` while Stripe dashboard is on `2026-04-22.dahlia`**: low practical risk since `checkout.session.completed` fields are stable, but SDK upgrade to v22 is queued for a future grooming build |
