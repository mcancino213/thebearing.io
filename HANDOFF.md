...
> Last updated: 2026-05-12 | Current build: v72t
...
| Build | Key changes |
|-------|-------------|
| v72t | **View transitions extended to partner + customer portals.** v72s fixed the navigation flash on admin pages by stripping `.html` from internal links (avoiding the Cloudflare Pages canonical-URL redirect that was silently skipping view transitions). v72t applies the same treatment to the other two portal experiences. **Changes:** (1) Renamed `assets/admin-chrome.css` → `assets/portal-chrome.css` with a portal-agnostic transition name (`portal-sidebar` instead of `admin-sidebar`); same CSS, broader scope. (2) Linked `portal-chrome.css` from all 33 portal pages: 11 admin (replacing the old admin-chrome.css link), 13 partner (`pp-*.html`), and 9 customer (my-account, bookings, conversations, preferences, saved, settings, lens, channels, search). (3) Stripped `.html` from 245 internal portal hrefs total: 175 partner + 12 admin JS-built refs + 58 customer + 3 partner JS redirect URLs in pp-login. (4) Updated badge scripts (`customer-sidebar-badges.js`, `pp-sidebar-badges.js`) to match both old and new URL forms when locating sidebar links to attach unread badges — graceful handling of cached or remembered URLs. **Out of scope:** public-site marketing pages (property pages, journal, hotels/villas/cruises listing pages, etc.) still use `.html` links and don't get the cross-fade treatment. Public→portal jumps (e.g. clicking "My Account" from a property page) will still flash — they go from a non-opted-in page to an opted-in one, so transitions can't fire. Within-portal navigation is the actual UX pain point that was addressed |
