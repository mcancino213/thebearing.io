...
> Last updated: 2026-05-15 | Current build: v73al
...
| Build | Key changes |
|-------|-------------|
| v73al | **Partner per-property email recipients (Part A).** Each property now has a `partner_emails: [...]` array on its record, edited via a new chip-list UI in admin-property-editor Section 0 (commercial terms). The worker's existing 6 email-sending paths (new enquiry, guest reply / change request, inbound email reply, offer declined, deposit paid, stale-conversation 24h/48h/72h reminders, legacy direct booking) now resolve recipients via a new `loadPartnerRecipients(slug, env)` helper. Helper reads `property.partner_emails`, falls back to `PARTNER_EMAIL_TRANSITION_DEFAULT = 'NourElNilTest213@gmail.com'` when missing/empty so notifications are never lost during migration. Logs `console.warn` on every fallback so unset properties are findable in CF logs. Each partner email is sent as a `[PARTNER]` -prefixed copy, deduped against `loadNotificationRecipients(env)` to avoid duplicate emails when partner inbox is also admin's. Per-conversation `conv.notifyPartner !== false` still suppresses (existing per-thread mute toggle from v62 era). **New admin-property-editor UI:** chip-list input under "Partner contact emails" in Section 0, with add/remove/Enter-to-add. Empty state shows amber warning copy explaining the transition default. **New admin-properties pill:** orange "No partner email" warning pill next to each property card's commission pill when `partner_emails` is empty; grey "N partner emails" pill (with full list in tooltip) when set. **Worker validation:** `POST /api/property` now validates `partner_emails` is a properly-formed array of emails (lowercased, deduped, regex-validated) and rejects malformed payloads; logs warning on saves with empty array. **To remove the transition default later:** (1) set partner_emails on every property, (2) delete `PARTNER_EMAIL_TRANSITION_DEFAULT` constant + fallback branches in `loadPartnerRecipients`. **Part B deferred to v73am:** universal per-partner notification toggle in partner-settings + per-booking/per-conversation toggles. Today's build sends all 6 operational notifications to partners unconditionally (modulo the existing per-conv `notifyPartner` flag) |
