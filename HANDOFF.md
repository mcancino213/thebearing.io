...
> Last updated: 2026-05-15 | Current build: v73an
...
| Build | Key changes |
|-------|-------------|
| v73an | **Three connected fixes from v73am follow-up + new primary "Build offer" CTA on partner enquiry emails.** Miguel reported four bugs from the v73am test: (1) partner email reply didn't trigger customer's unread badge; (2) partner reply didn't reach the customer's email; (3) reply appeared in the conversation thread under the CUSTOMER's name; (4) partner email's only CTA was "open conversation" when the real action item is to build an offer. **Root cause of #1-#3 — single bug:** `PARTNER_EMAIL_TRANSITION_DEFAULT` was set to capital `'NourElNilTest213@gmail.com'` in v73al, but the inbound classifier lowercases the `fromEmail` and uses `indexOf` against the partner list. `['NourElNilTest213...'].indexOf('nourelniltest213...')` = -1 = no match = `role:'guest'`. Cascading failures: stored msg.role wrong (#3), customer-notify branch never executed (#2), `conv.unreadGuest` never incremented (#1). **Fix:** lowercased the constant to `'nourelniltest213@gmail.com'`. Also added defensive `.toLowerCase()` on the partner list inside the classifier so this can't recur if anyone hand-edits a property record with mixed-case emails. Added a `[Inbound] classified as guest reply. fromEmail=X partnerList=[...]` log line on misses so misclassifications are findable in CF logs. **Fix #4 \u2014 Build offer is now the primary CTA on the new-enquiry partner email.** Switched that email from plain text to HTML with two buttons: terracotta "Build offer \u2192" (primary, deep-links to `pp-bookings?as={slug}&newOffer={bookingRef}`) and underlined "Open conversation" (secondary). New deep-link handler in `pp-bookings.html`: when URL has `?newOffer=X`, after the bookings list loads, auto-opens the offer-builder modal for that booking. One-shot guarded by `window._ppOfferAutoOpened`. If the ref isn't in the partner's list (wrong `?as=` slug), logs a console warning. Existing 7 partner email sites unchanged \u2014 only the new-enquiry one needed the new CTA, the rest are notifications of events that already happened. **Note:** existing test conversations with the misclassified `role:'guest'` partner messages stay that way; new partner email replies (post-v73an deploy) classify correctly. To clear out the test debris, just create fresh enquiries |
| v73am | Partner email replies route back to the conversation + portal links land on the right property. `reply_to` on partner emails, inbound classifier by from-email, `&as={slug}` injected into portal links, auto-open on `?id=` on both pp/admin conversations pages |
