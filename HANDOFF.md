...
> Last updated: 2026-05-12 | Current build: v72o
...
| Build | Key changes |
|-------|-------------|
| v72o | **admin-properties.html: dead prospecting code removed + listing-order regression fix.** Two cleanups in one build. **(1) Removed dead code:** the AI prospecting kanban (drag-to-pipeline funnel + AI email composer + Resend send + template gallery + reply tracker) was already non-functional — the kanban DOM was removed in v66 and never put back, so `render()` short-circuited on every page load. The supporting infrastructure (~470 lines of JS with hardcoded fake property names like Amangiri/Nihi Sumba/Mahali Mzuri, 4 dead modals, ~50 lines of unused CSS) was still in the source pretending to be a feature. Miguel does partner outreach manually, so the whole thing is gone. File shrunk 1281 → 786 lines (~40% reduction). What remains: the real Published/In Progress/Listing Order tabs reading from `/api/property`. **(2) Listing-order save regression:** v72m's slug validation on `/api/property` POST inadvertently blocked the legitimate `__index_hotels`/`__index_cruises`/`__index_villas` writes used by the Listing Order tab's "Save order" button. Worker now special-cases those three slugs, validates the payload shape (`{slugs: [...]}`), filters any nested junk slugs, and writes to both `{slug}:property` (legacy read path used by public listing pages) and `{slug}` (direct key, preferred going forward). All other `__*` slugs still rejected |
