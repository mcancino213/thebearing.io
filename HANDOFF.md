...
> Last updated: 2026-05-12 | Current build: v72r
...
| Build | Key changes |
|-------|-------------|
| v72r | **Fixed the actual cause of the navigation flash.** v72q added view transitions but the white flash persisted — root cause was `admin-gate.js` injecting `body { visibility: hidden !important; }` on every admin page load. View transitions was working correctly; it was animating a transition to a page that had hidden itself. **Fix:** admin-gate now skips the visibility-hide on subsequent loads in the same browser session, gated by a `tb_admin_verified_v1` flag set in sessionStorage after the first successful auth. First admin page load still hides until verified (security stance unchanged); every subsequent click within the admin portal shows the new page immediately, letting view transitions actually animate. On sign-out / auth failure the flag is cleared. Worker independently verifies admin status on every API call, so the brief window of "visible content while background re-checking auth" doesn't leak anything. **Secondary fix:** `admin-fetch.js` now awaits Clerk-ready before issuing `/api/` calls, preventing a race where pages that fetch immediately on load (admin-settings, admin-analytics) would send requests with no admin headers and get 403'd. 15s cap on the wait so a stuck Clerk init can't deadlock fetches forever |
| v72q | **Cross-document view transitions for admin pages.** New shared stylesheet `assets/admin-chrome.css` containing `@view-transition { navigation: auto; }` and `.sidebar { view-transition-name: admin-sidebar; }`. All 11 admin pages link the stylesheet. (Initially shipped without fixing the body-visibility issue that prevented transitions from being visible — see v72r for the actual working fix) |
