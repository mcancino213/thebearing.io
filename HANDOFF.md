...
> Last updated: 2026-05-13 | Current build: v72z
...
| Build | Key changes |
|-------|-------------|
| v72z | **Critical fix: property.html had an unclosed `populateConfirmation()` function that broke the entire page.** Single missing `}` at the end of the function (file line 2156) caused the whole inline `<script>` block to fail parsing with `Uncaught SyntaxError: Unexpected end of input`. With JS broken, `loadProperty()` never ran → the page hung on the "Loading…" spinner forever. **This bug was NOT a v72y regression — `node --check` confirms it has been present in every build going back to at least v72o.** It only surfaced now because Miguel hadn't visited property pages "in some time" (paraphrased from his words) and recent work has been on portals. The bug was probably introduced during the v56-v57 series of property-page rebuilds and went unnoticed because the page often appears to start loading before the JS error fires. **Diagnosis trail:** Miguel reported perpetual loading on `/property?slug=gypsy-by-mekong-kingdoms`. DevTools console showed `Uncaught SyntaxError: Unexpected end of input` at property.html:3015 (end of script block). Bisecting which section caused the imbalance pinpointed `populateConfirmation()` at line 2078. Function declared an inner `createNewConversation()` (line 2132), which had its closing `}` at line 2156, but `populateConfirmation` itself was never closed. Added the missing `}` directly after line 2156 with a comment explaining why. **No other changes in v72z** — this was a single-line surgical fix shipped immediately so v72y's offer flow can be smoke-tested. v73a will continue with member-side offer review + Stripe Checkout |
