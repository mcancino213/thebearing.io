...
> Last updated: 2026-05-13 | Current build: v73h
...
| Build | Key changes |
|-------|-------------|
| v73h | **"Your Cove" → "Your Compass" rename + signed-out user can now navigate back to home.** Six brand references in `my-account.html` and `conversations.html` replaced (3 in each: page-load failsafe, signed-out overlay, Clerk-load-error fallback). "Cove" was the site's old name and shouldn't appear anywhere; "Compass" fits The Bearing's navigation/direction motif and pairs with "Your" naturally (your personal compass for curated travel). All six signed-out screens now include (a) a clickable "The Bearing" wordmark above the "Your Compass" heading that links to `/`, and (b) a discreet "← Back to The Bearing" link in terracotta below the action area. Previously a signed-out user landing on `/my-account` or `/conversations` was overlay-locked with no way back to the home page except manually typing the URL. The signed-out main overlay also now has `padding:32px 16px; overflow-y:auto` so on short viewports the Back link doesn't get clipped. Validated: no remaining "Your Cove" / "your cove" / "YOUR COVE" anywhere in the codebase. The other customer-portal pages (bookings, lens, saved, preferences, settings) weren't touched — they render normally for signed-out users (with placeholder data) and have full nav chrome with home links built in, so they're not stranding scenarios |
