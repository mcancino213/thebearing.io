...
> Last updated: 2026-05-13 | Current build: v73i
...
| Build | Key changes |
|-------|-------------|
| v73i | **Three fixes: second-enquiry silent-discard bug + booking backfill + auth modal centering (4th attempt).** (1) **Second enquiry to same property no longer disappears silently.** The single most impactful bug in this build chain — Miguel's second Gypsy enquiry vanished from chat AND customer side. Root cause: `submitEnquiryAndOpenChannel` (since v73b) detected an existing open conversation for the same property and **redirected the user without sending their new message**. The new dates/room/notes were discarded entirely. The conversation creation worker was never called, no booking was created, nothing changed in KV. Fix: when an existing open conversation is found, post the new enquiry as a `action:'message'` follow-up to that conversation, AND include `updateEnquiry={arrival,departure,guests,cabin,notes}` so the conversation's enquiry shape (visible on partner side panel) reflects the latest dates. The linked booking record (if any) also gets updated dates/room. Then redirect to the conversation. Every submit now produces a visible message in the right channel — never silent discard. (2) **Backfill endpoint + admin button for stub bookings.** `/api/conversation/backfill-bookings` (admin-gated, POST) walks `__conversations_index` and creates a `booking:{ref}` stub with `status:'enquiry'` for every conversation that lacks one. This fixes the "old enquiries don't appear in pp-bookings" problem since v73g's auto-stub-creation only applies to NEW conversations. New "Backfill bookings from conversations" button in admin-settings Maintenance section. Idempotent (skips conversations that already have `bookingRef`). Also added `console.log` lines in the auto-create path so future debugging via Cloudflare worker logs is easier. (3) **Auth modal centering — fourth attempt, defensive.** Switched from flex (`align-items:flex-start; justify-content:center`) to CSS Grid (`place-items:start center`) with explicit `width:100vw; height:100vh; top:0; left:0` on the overlay. Sheet got `margin:0 auto` as a horizontal-centering backup. The combination should work in every modern browser even if one of the layout methods misbehaves. **Caveat:** without a fresh screenshot from Miguel showing exactly what was wrong, this is still a partial guess. If the modal is STILL not centered after v73i, please send a screenshot + DevTools console output so I can stop speculating |
