...
> Last updated: 2026-05-14 | Current build: v73af
...
| Build | Key changes |
|-------|-------------|
| v73af | **Inline payment now confirms booking without depending on the webhook + offer dates backfill conv.enquiry.** Two real issues from Miguel's smoke test: (1) Stripe inline payment succeeded but the booking stayed in `offer_sent` status \u2014 was actually re-payable. Root cause: the Stripe webhook endpoint is subscribed only to `checkout.session.completed`, not `payment_intent.succeeded`. The inline flow fires PI-succeeded only. Webhook never received the event, booking never updated. (2) Conversation list dates didn't appear for conversations where the customer didn't originally specify dates but the partner's offer did establish them. **Fixes:** (a) **New `/api/checkout/sync-payment` endpoint** \u2014 client-initiated booking sync. After `stripe.confirmPayment()` succeeds client-side, `stripe-inline.js` now POSTs the PI ID here. Worker verifies the PI with Stripe (real source of truth, not client claim), then runs the same booking-confirmation flow as the webhook: mark `status=confirmed`, snapshot offer details onto booking, post system message, recompute unread counters. Idempotent with the webhook \u2014 whichever lands first wins, the other no-ops via the existing `booking.paymentStatus === 'deposit_paid' && booking.stripeSessionId === pi.id` guard. (b) **`stripe-inline.js` updated** \u2014 instead of just calling `onSuccess` after `confirmPayment()`, it now awaits the sync-payment call before redirecting. If sync fails, surfaces an honest error message ("Payment received but we could not finalise on our side, contact admin@thebearing.io with reference pi_X") rather than redirecting to a misleading "confirmed" page. (c) **New admin-gated `/api/admin/sync-stripe-payment` endpoint + admin-settings Maintenance button** \u2014 paste a PaymentIntent ID from Stripe dashboard, click Sync. Same processing but bypasses the customer email check. Use this to repair Miguel's existing stuck v73ab test booking (find the PI ID in Stripe dashboard, paste, sync). Skips duplicate `booking_confirmed` system messages if one already exists for that booking. (d) **`postOfferCardToConversation` now backfills `conv.enquiry` from the offer** when fields are empty. So if a customer enquired without specifying dates and the partner's offer establishes them, the conv list date subline now populates after the offer is sent. Only fills empty fields \u2014 never overwrites a customer-supplied value. **Operator action required:** add `payment_intent.succeeded` to the Stripe webhook endpoint subscription in the Stripe dashboard. Even with sync-payment working, the webhook is the right primary path \u2014 it fires even if the customer's browser dies mid-redirect. With both paths, payment confirmation is bulletproof |
