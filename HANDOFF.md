...
> Last updated: 2026-05-12 | Current build: v72x
...
| Build | Key changes |
|-------|-------------|
| v72x | **`commission_pct` + `booking_type` fields added to property schema.** Two new top-level fields on every property record, with admin UI + worker validation + on-read backfill for legacy records. **Worker:** (1) `GET /api/property?slug=X` now backfills `commission_pct: null` and `booking_type: "enquiry"` on records that lack them (idempotent, read-only, no KV writes — defaults apply in-memory before the response is sent). Means Gypsy/Singita don't break when loaded after this build. (2) `POST /api/property` validates the two fields: `commission_pct` must be `null` or a number 1–25; `booking_type` must be `"enquiry"` (the only allowed value in v1; `"instant"` reserved for future PMS integration). Out-of-range values return 400 with a clear error. Both are optional in the payload — admins can edit other fields on a property without commission set, the worker won't block. **admin-property-editor:** New "Section 0: Commercial terms" at the very top of the Listing tab (above Core identity), marked "Admin only." Two fields: commission percentage (number input with % suffix, 1-25, inline validator showing dollar example on a $5,000 booking) + booking type select (`"Enquiry → offer → accept"` selected; `"Instant book"` disabled with explanatory text). `updateJSON()` writes both as top-level fields in the property JSON. `propLoad` belt-and-suspenders syncs both fields from top-level after `propRestore` runs (so they populate even if the snapshot doesn't have them). **admin-properties:** Each property card now shows a commission pill — gray "15% commission" when set, amber "Commission not set" with attention dot when null. **Member impact:** none. Commission percentage is internal/admin-only — never shown to members or on property pages. **v72z prerequisite:** the Stripe checkout endpoint coming in v72z will refuse to create sessions for properties with `commission_pct === null`. That's where the field becomes truly required, not at save time |
