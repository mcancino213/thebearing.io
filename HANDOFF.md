...
> Last updated: 2026-05-12 | Current build: v72v
...
| Build | Key changes |
|-------|-------------|
| v72v | **admin-payments dummy data killed + payments architecture locked.** Replaced the hardcoded $2.41M GMV / $361k commission / fake transactions table / fake disputes panel with an honest empty state. Page now shows a status banner explaining Stripe isn't wired yet, four KPI tiles with em-dash placeholders, an empty transactions card, and a roadmap panel describing the v72v→v72z build sequence. **Booking model decisions made this build (impact all subsequent payment builds — see PROJECT_CONTEXT.md):** (1) **Deposit-only model.** TheBearing collects only its negotiated commission percentage as a non-refundable booking fee at the moment of member acceptance. The partner arranges the trip balance directly with the member. TheBearing is never merchant of record for the full trip — tax-clean, no Stripe Connect partner onboarding required, works regardless of partner country. (2) **All offers and charges in USD.** Partners enter trip prices in USD; member pays in USD; member's card converts at their bank's FX rate. No FX risk in TheBearing's data model. Partner is responsible for translating their local-currency pricing into USD when building offers. (3) **Per-property commission percentage** — negotiated per partner, stored as `commission_pct` field on property records (UI in v72w). No default; properties without a set commission can't be published once v72w ships. (4) **Enquiry-only booking flow for v1.** Every property operates in enquiry mode until partner-specific instant-book is needed. Member sends enquiry → partner builds offer in portal → member accepts and pays deposit → partner contacts member for balance. Channel-manager / PMS integrations deferred. (5) **Pricing supports both per-night × nights AND package total** — partner chooses per offer. Single nightly rate, no mid-stay rate changes for v1. (6) **Offer revisions, not counter-offers** — members can request changes (room, dates, inclusions), partner builds a new offer that supersedes the previous. No price haggling. No structural changes to KV or worker this build — just dead-data cleanup and decisions documented |
