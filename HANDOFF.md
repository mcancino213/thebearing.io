...
> Last updated: 2026-05-13 | Current build: v73n
...
| Build | Key changes |
|-------|-------------|
| v73n | **/bookings page CSS hotfix — v73m shipped with an unclosed `.bk-section-pill {` selector that broke ALL v73m styles.** When I str_replaced the v73k pill CSS to insert the new color-coded pill rules in v73m, I targeted the rule body but didn't include the opening selector line in my old_str pattern, so I left the orphan opener `/* v73k: bookings-page sections + cards */\n  .bk-section-pill{` immediately followed by my new comment and selectors. The browser interpreted `.bk-section-pill { /* comment */ .bk-pills-row { ... } }` as a malformed nested rule, silently abandoned parsing, and the entire v73m CSS — pills, section panels, tints, offer card overrides, cancel button styling — failed to apply. Symptom: /bookings page rendered as default browser styling, blue underlined hyperlinks for pills, no panels, no tints. **Self-correction: brace counts can pass while CSS is still broken if the wrong opener pairs with the wrong closer.** Should have visually inspected the join site, not just run a brace counter. Also discovered the file has had a separate pre-existing malformed `@keyframes ciDot` declaration (duplicated `40%{}` clause) since well before v73m — browsers were silently skipping that single rule via CSS error recovery, but my v73m bug created a cascading parse failure that error-recovery couldn't escape. No code changes other than removing the orphan opener |
