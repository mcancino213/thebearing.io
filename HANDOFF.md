...
> Last updated: 2026-05-13 | Current build: v73p
...
| Build | Key changes |
|-------|-------------|
| v73p | **Chevron hotfix that should have shipped with v73o.** v73o introduced the cancelled section on /bookings with a `<span class="bk-cancelled-chev">\u203a</span>` markup — a JavaScript-style unicode escape inside raw HTML, which renders as literal text "\u203a" instead of the chevron character `›`. Same class of bug as v73l's "\u2192" arrow issue. **Process failure:** I made this fix in the working folder shortly after Miguel reported it, but then asked clarifying questions about Stripe integration instead of packaging and shipping. The user was sitting on the broken v73o zip the whole time. v73p is just the packaged fix — single character change: literal `›` (UTF-8 bytes 0xE2 0x80 0xBA) in the markup. Validated via `od -c`. **Lesson logged**: when a fix is in the working folder, ship it before pivoting to new work. Asking questions about the next thing while shipping the previous fix is fine; asking questions while the previous fix is still unshipped is not |
