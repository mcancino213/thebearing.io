...
> Last updated: 2026-05-13 | Current build: v73b
...
| Build | Key changes |
|-------|-------------|
| v73b | **Property.html universal-template hardening: 3 fixes.** All three issues Miguel flagged when enquiring on Gypsy by Mekong Kingdoms but ending up in his Nour El Nil conversation. **Root cause (the big one):** property.html had a leftover `kvHydrate()` IIFE (line 2596 in v73a) from when the file was Nour El Nil's standalone page. The IIFE hardcoded `var SLUG = 'nour-el-nil'` and fetched Nour El Nil's KV data on EVERY property page load, then overwrote `.prop-name`, `.prop-location`, `.prop-intro`, `.stats-strip` etc. — race-condition-overwriting whichever property `loadProperty()` had just rendered. The visible "Gypsy" header survived only because Gypsy's KV record lacks the `render` shape kvHydrate needs to fully overwrite. Meanwhile `populateConfirmation()` and the booking-checkout flow at line 2544 both read `var propSlug = (typeof SLUG !== 'undefined') ? SLUG : ''` — but the IIFE's `var SLUG` was function-scoped (not visible globally) so `propSlug` was always `''`. Result: every enquiry POSTed to `/api/conversation` had `propertySlug: ''`, and the "is there already an open conversation?" check at line 2120 matched on `propertySlug === ''` — finding any older orphan conversation (e.g. Miguel's previous Nour El Nil enquiry) and redirecting there. **Fixes:** (1) `loadProperty()` now stashes the URL slug as `window.PROPERTY_SLUG` early, before any flow can read it. (2) `populateConfirmation()` and the booking-checkout flow read from `window.PROPERTY_SLUG` instead of the unreliable `typeof SLUG` pattern. (3) Added a guard: if `propSlug` is empty, refuse to create the conversation rather than silently saving an orphan. (4) Deleted the obsolete ~100-line `kvHydrate` IIFE entirely — `loadProperty()` and `renderProperty()` are the only hydration path now. (5) Updated Step 3 banner to dynamically show the actual property name via `#inq-conv-name`. **Cleaned up Step 3 UI:** Replaced the entire fake-iPhone-conversation preview (hardcoded "Hi — I'd like to inquire about the Deluxe Suite on Nour El Nil for 2 guests, departing 15 October 2025" etc., the fake auto-reply, the fake booking confirmation card with Nile dates) with a clean confirmation panel: "Your request has been sent." headline, a 3-step "What happens next" list, and two working action buttons. The terracotta CTA is now a real `<a>` element whose `href` is set to `/conversations.html?id={newId}` by `populateConfirmation()` after the conversation is created — actually navigates instead of just toasting and closing |
| v73a | **Three property.html enquiry-form fixes.** Replaced single free-text `inq-guests` field with side-by-side `inq-adults`/`inq-children` number inputs (matched what populateReview/populateConfirmation expected to read). Fixed "Open my conversation" button calling undefined `submitEnquiryAndGo()` — now calls `goStep(3)` which creates the conversation in KV. Cleaned orphaned duplicate `<svg>` + stray `</a>` on Step 3 |
| v72z | **Critical fix: property.html had an unclosed `populateConfirmation()` function** that broke the entire page. Single-line surgical fix. Bug pre-dated v72y |
