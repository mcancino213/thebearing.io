...
> Last updated: 2026-05-12 | Current build: v72q
...
| Build | Key changes |
|-------|-------------|
| v72q | **Cross-document view transitions for admin pages — eliminates the navigation flash.** v72n's sidebar partial only stopped the sidebar markup from re-fetching on each click; the actual visual flash between admin pages (browser tearing down the old document and rendering the new one) was untouched. **Fix:** new shared stylesheet `assets/admin-chrome.css` containing `@view-transition { navigation: auto; }` (opts the document into cross-document transitions) and `.sidebar { view-transition-name: admin-sidebar; }` (marks the sidebar as a shared element so the browser preserves it across navigations instead of fading it out and in). Default cross-fade speed shortened to 140ms — fast enough to feel responsive in an admin tool. All 11 admin pages now `<link>` the shared stylesheet. **Note:** I initially recommended this fix in v72n, then talked Miguel out of it in favor of the sidebar partial — wrong call, because the partial fixed sidebar re-rendering but did nothing for the page-to-page flash he actually saw. The sidebar partial is still valuable (less duplicated HTML, faster sidebar paint) but the visual fix is this one. Supported in Chrome 126+, Safari 18+, Firefox 145+; older browsers fall back silently to today's behavior |
