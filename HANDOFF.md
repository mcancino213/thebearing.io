...
> Last updated: 2026-05-14 | Current build: v73y
...
| Build | Key changes |
|-------|-------------|
| v73y | **Six fixes from v73x Stripe smoke test feedback.** (1) **Receipt wording updated** on the persistent green confirmed-booking card: "The remaining balance is settled with the property at the time of stay." \u2192 "The remaining balance is settled directly with the property. They will be in touch with details when the balance is due." (2) **Partner sidebar bookings badge** now counts only items needing partner action, not all bookings. Previously the badge was always-on showing the total non-cancelled count, which was noisy. New filter: bookings where `status:'enquiry' && !active_offer_id` (new enquiries) OR `pendingChangeRequest` (guest requested change). Confirmed bookings and offer-sent (waiting on guest) states are NOT counted because there's no partner action required. (3) **Room/package dropdown in offer builder** \u2014 swapped the freetext input for a `<datalist>`-backed input that surfaces the property's rooms as tappable options while still accepting custom text for charters / special packages. Populated on modal open from `property.rooms[]`. Capacity hint included in label when available. (4) **Customer-side flag when offer room differs from enquiry room.** Snapshot `enquiry_cabin` onto offer at creation time (worker line 3092) so the customer's offer-detail card can compare offer.room vs the original enquiry's cabin and render a sand-colored "Note: the property is offering X instead of the Y you asked about" inline note. Comparison is case-insensitive + trimmed for fuzzy matching ("Deluxe King Suite" vs "deluxe king" doesn't trigger false flag). Falls back to `conv.enquiry.cabin` for offers created before v73y. Also added immutable `enquiry_snapshot` field on the booking record at enquiry creation \u2014 captures arrival/departure/guests/cabin/notes at the moment of original ask, never overwritten when offers are sent/revised, so the true "what they originally asked for" is always available regardless of subsequent offer-driven mutations. (5) **Partner conversations badge not updating after deposit payment** \u2014 root cause: Stripe webhook writes the "booking_confirmed" system message and bumps `conv.unreadAdmin`, but does NOT call `recomputeUnreadCounters(env)` afterward. The unread-count endpoint reads from a cached `__unread_counters` KV key that's only refreshed by explicit recompute calls; every other writer in the worker (admin reply, decline action, etc.) calls recompute, but this one was missing. Fixed: added the call right after the conversation write. Badge will now reflect new messages from Stripe webhooks. (6) **Timing instrumentation on customer offer modal load** \u2014 Miguel reported ~10 second load time on one occasion. Added `performance.now()` markers and console.log lines at fetch-start / response-received / json-parsed boundaries. Next time it's slow, the console will show whether the bottleneck is network latency, worker cold-start, or something downstream. No actual fix yet because we don't know the cause; instrumentation is the diagnostic, not the cure |
