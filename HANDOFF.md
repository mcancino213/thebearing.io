...
> Last updated: 2026-05-15 | Current build: v73ak
...
| Build | Key changes |
|-------|-------------|
| v73ak | **Nights count computed from dates when not stored on booking record.** Miguel caught pp-bookings showing "May 22 \u2013 May 29, 2026 \u00b7 ? nights" on an enquiry with perfectly good dates. Root cause: the enquiry form on the customer side doesn't compute nights when arrival + departure are known; the field is left empty. Pre-v73ai bookings also wouldn't pick up nights from offers. `fmtDateRange()` in pp-bookings.html and the inline date cell in admin-bookings.html both fell back to literal '?' when `b.nights` was falsy. **Fix:** both date renderers now compute nights from `(departure - arrival)` if not provided. Defensive only-when-positive guard prevents weird negative/zero outputs. Applied to `fmtDateRange`, `fmtDateRangePlain` (used for embed in change-request pills), and admin-bookings inline render. No worker changes \u2014 this is a presentation-layer fix. Going forward (post-v73ai) bookings will have proper `nights` baked in from offer-send, but legacy records get this graceful fallback |
