...
> Last updated: 2026-05-12 | Current build: v72n
...
| Build | Key changes |
|-------|-------------|
| v72n | **Admin sidebar consolidated to a cached partial — no more flash on nav.** Every admin page used to ship its own ~3.6kB copy of the sidebar markup. Clicking a menu item triggered a full page navigation, which re-parsed and re-rendered the sidebar — visibly flashing on every click. **Fix:** extracted the sidebar HTML to `assets/admin-sidebar.html`. New `assets/admin-sidebar-inject.js` runs as early as possible (right after the empty `<aside id="admin-sidebar-mount">`), reads `sessionStorage['tb_admin_sidebar_v1']`, and injects synchronously on cache hits. First-ever load in a session fetches the partial once (~40ms), caches it, and re-runs the badge + user-name hooks. Every admin page now has `<body data-page="X">` so the inject script can mark the right link active. Net: −40kB across the 11 admin pages, no flash on subsequent nav. To change sidebar markup later, edit `assets/admin-sidebar.html` AND bump `SIDEBAR_VERSION` in `admin-sidebar-inject.js`. New helper: `window.refreshAdminSidebarUser()` (exposed by `admin-sidebar-user.js`) for late-mount re-init |
