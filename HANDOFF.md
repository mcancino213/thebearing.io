...
> Last updated: 2026-05-14 | Current build: v73v
...
| Build | Key changes |
|-------|-------------|
| v73v | **Email delivery diagnostics.** Miguel reported no admin emails arriving lately. Built two surfacing tools to localize the failure rather than guess. (1) **Resend health check now surfaces sender-domain verification status.** Previously the System Health card's Resend row only said "connected" if the API key worked. That misses the most common failure mode: API key valid but `thebearing.io` not verified in Resend, so every `from:bookings@thebearing.io` send returns 403 and the worker's `.catch()` swallows it silently. New behavior: `/api/health` now walks the domain list returned by `/v1/domains`, finds the `thebearing.io` entry, and surfaces its status. Three explicit outcomes: "thebearing.io verified \u2014 sending OK" (green), "thebearing.io status: pending \u2014 emails from bookings@thebearing.io WILL FAIL" (red with explicit doom statement), "thebearing.io NOT in configured domains \u2014 every send will fail" (red). (2) **New "Send test email" button in admin-settings Maintenance.** Fires a single Resend send to `loadNotificationRecipients(env)` (admin baseline plus any extras) and returns Resend's response in full — HTTP status, email id on success, error message on failure. Lets admin confirm end-to-end delivery without going through a booking flow. The output pane explicitly suggests next steps if the email doesn't appear (check Spam/Promotions, Gmail search `from:bookings@thebearing.io`, Resend dashboard logs). New endpoint: `POST /api/test-email` (admin-gated). **Three possible root causes that v73v will narrow down**: (a) `thebearing.io` not verified in Resend — health card will say so; (b) `notifyAdmin: false` on the test conversation — `/api/test-email` bypasses this entirely and will deliver, proving the pipe works; (c) emails being delivered to Spam/Promotions — Resend dashboard will show successful delivery. **Deferred:** the partner email system (per-property `partnerEmail` field + routing) is queued for v73w after Miguel finishes the Stripe smoke test. Partner notifications currently route to `admin@thebearing.io` as a placeholder per the original v73q scope note |
