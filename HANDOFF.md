...
> Last updated: 2026-05-14 | Current build: v73w
...
| Build | Key changes |
|-------|-------------|
| v73w | **Email infrastructure: split reply receiving onto `replies.thebearing.io` subdomain.** Diagnosed in v73v session that emails to `admin@thebearing.io` were going into a void: root `thebearing.io` MX records pointed at Resend's inbound infrastructure (for the customer reply-into-conversation flow), which meant Workspace's `admin@thebearing.io` user could never receive external mail. Resend showed every email as "Delivered" but they were being delivered to Resend's own inbound MX, not to Gmail. Decision: keep reply-into-conversation feature alive on a subdomain so root MX can flip to Google. (1) Added `replies.thebearing.io` as a verified domain in Resend, both sending and receiving enabled. DNS records set up in Cloudflare: TXT for DKIM at `resend._domainkey.replies`, MX + TXT at `send.replies` (Resend's outbound feedback), MX `replies` priority 10 to `inbound-smtp.us-east-1.amazonaws.com` (subdomain inbound). (2) Worker change: `reply_to` in conversation notification email now reads `reply+${id}@replies.thebearing.io` instead of `@thebearing.io`. One-line change at envoy.js:1930. (3) Worker change: inbound webhook regex at envoy.js:2263 broadened to match BOTH `@thebearing.io` and `@replies.thebearing.io` recipients — so during the MX cutover (Phase 5, next step after v73w deploys + tests), any in-flight emails sent under the OLD reply-to still parse correctly even if they happen to arrive after MX flip. Belt-and-suspenders. **Not yet done (deferred to manual DNS work after v73w testing):** flipping root `thebearing.io` MX from Resend's inbound-smtp records to Google's ASPMX servers. Required for `admin@thebearing.io` Gmail to receive external mail. Won't be done until Phase 4 test confirms the new subdomain reply flow works end-to-end. **What this unlocks once Phase 5 is done:** admin@thebearing.io receives mail normally, transactional notifications land in inbox, Stripe smoke test finally has a verified email leg, /api/test-email diagnostic from v73v will show "Sent to admin@thebearing.io" and the email will actually arrive |
