...
> Last updated: 2026-05-14 | Current build: v73ab
...
| Build | Key changes |
|-------|-------------|
| v73ab | **Two things: (a) fixed a sneaky regression from v73aa where the desktop sticky card's "Enquire to book" button was still wired to the checkout flow despite having `onclick="openEnquiry()"`, and (b) shipped Stripe inline (Payment Element).** **The regression:** property.html line 1733-1748 had a post-hydration JS block that called `document.getElementById('dbc-book-btn').onclick = function(){tbOpenCheckout(...)}` once the property data loaded, **overriding** the inline onclick. So my v73aa change to the inline handler was correct but useless \u2014 the JS rebinding still made the button open the checkout/payment flow. The bug was visible because the button showed real prices and a "pay deposit" CTA. Fix: removed the post-hydration override entirely. The inline `onclick="openEnquiry()"` is now authoritative. Also fixed the cabin-card "Select" buttons (line 1756) which had the same problem \u2014 they were generating dynamic `onclick="tbOpenCheckout({...})"` strings. Now they call `openEnquiry()`. Also swept `vibe-search.html` result cards which had a Book now button still wired to checkout \u2014 removed entirely (View property button remains, that's the right next step). The 12 old demo property pages (amangiri, alila-jabal-akhdar, etc.) also have Book now buttons but are deprecated per project memory \u2014 not touched. The `tbOpenCheckout` and `tbAuthBook` function definitions are kept in place as dead code rather than ripped out \u2014 minor risk of breaking something we don't know about, easy enough to clean up later. **Stripe inline (Payment Element):** customer no longer redirects to checkout.stripe.com when paying the deposit. New flow: click Accept & pay \u2192 fullscreen modal opens with Stripe's Payment Element embedded \u2192 enter card \u2192 `stripe.confirmPayment()` \u2192 success or 3DS challenge (handled by Stripe iframe overlay) \u2192 redirect to /bookings?checkout=success which triggers the persistent green hero card from v73x. **Architecture:** new worker endpoint `/api/checkout/create-intent` returns a PaymentIntent client_secret + publishable key. Mirrors create-session's auth + validation; differs only in what it returns. The OLD `/api/checkout/create-session` endpoint stays in place as fallback (unused but kept). Webhook now handles BOTH `checkout.session.completed` AND `payment_intent.succeeded` via a single normalization block at the top of the handler \u2014 the PI is reshaped to look like a Checkout Session and the existing handler runs unchanged. PIs are tagged with `metadata.inlineFlow='true'` so the webhook can tell them apart from PIs that came via Checkout (those are processed via the session event to avoid double-processing). New client-side asset: `assets/stripe-inline.js` exposes `window.TBStripeInline.payDeposit({offerId, offerLabel, onSuccess, onCancel, onError})`. Included on bookings.html and conversations.html. Loads Stripe.js (`js.stripe.com/v3`) lazily on first invocation. Modal styling matches the rest of the app (cream background, Instrument Serif headers, terracotta CTA). **Requires `STRIPE_PUBLISHABLE_KEY` in Cloudflare Pages env** \u2014 will surface a clear 503 error if missing |
