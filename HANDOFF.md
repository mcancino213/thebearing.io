...
> Last updated: 2026-05-15 | Current build: v73ah
...
| Build | Key changes |
|-------|-------------|
| v73ah | **Five fixes from Miguel's smoke test of v73af/v73ag.** (1) **"Previously declined offer" pill spacing on pp-bookings** \u2014 was rendering inline next to "Dates TBD" with no separation. Root cause: `<div class="declined-pill">` with `display:inline-flex` was being absorbed inline next to the inline "Dates TBD" `<span>` because the parent `<td>` was in inline flow with no break. Fix: wrapped the pill in a block-level `<div style="margin-top:6px;">` so it forces a new line. Same fix applied preemptively to the `pcr-pill` for pending change requests. (2) **Custom dropdown component for Room/package** \u2014 the v73y `<datalist>` approach had no clickable arrow (native browser limitation) and didn't open on click in most browsers. Replaced with a fully custom `.tb-select` component: styled trigger button with animated terracotta chevron, options list populated from `property.rooms[]` with capacity hints, "Other / custom\u2026" option that swaps in an inline text input for charters and special packages, click-outside-closes, Escape-closes, selected-state highlighting. The hidden `#om-room` input remains the source of truth for read/write code; the dropdown writes into it. Three prefill paths updated (initial booking values, prior offer on revise, pending change request override) to route through new `setRoomDropdownValue()` helper. (3) **Offer valid + Balance due alignment** \u2014 the two date inputs in the form-row were vertically misaligned because the right column's hint subtext "optional \u2014 when guest pays the remaining balance" wrapped to 2 lines while the left's "defaults to 7 days" fit on 1. Fix: made `.field` a flex column and added `align-items:end` to `.form-row` so inputs anchor to the bottom of their grid cells regardless of label/hint height. Inputs now align across columns even when labels are different heights. (4) **`seenByPartner` / `seenByAdmin` flags + Bookings sidebar badges** \u2014 partner and admin had no notification when a new booking confirmed. Conversations badge already covers it via the deposit-paid system message, but that's not enough \u2014 you want visibility in Bookings nav too. **New behavior:** all three confirmation paths (Stripe webhook, customer sync-payment, admin sync-payment) now set `seenByPartner=false` and `seenByAdmin=false` on the booking record. Partner sidebar badge (was: only new enquiries + change requests) now also counts `status:'confirmed' && seenByPartner !== true`. Admin sidebar got a new Bookings badge slot in `admin-sidebar.html` (SIDEBAR_VERSION bumped 2\u21923 for cache invalidation), wired in `admin-sidebar-badge.js` (file now drives BOTH Conversations and Bookings badges, with `.sb-badge:not(.sb-badge-bookings)` selector to avoid collision). Polls `/api/booking` every 30s for admin. **New endpoint:** `POST /api/booking/mark-seen` with `{ ref, role: 'partner' | 'admin' }`. Admin role gated by `isAdmin()`. Partner role open (real partner auth is on deferred list). (5) **Confirmed booking detail modals on both portals** \u2014 the partner-side View button was a disabled placeholder; the admin-side had no View at all. Both now show a styled fullscreen read-only modal with guest+stay header, total / deposit-paid / balance-owing financial grid, inclusions checklist, exclusions, cancellation terms, partner notes, guest enquiry notes. Reads from the snapshot fields set on the booking at confirmation time (v73aa). For pre-v73aa bookings without snapshot data, shows a sand-colored "Some details unavailable" notice. Opening the modal fires `/api/booking/mark-seen` which clears the unseen indicator dot (small terracotta dot next to the View button) and refreshes the sidebar badge. Modal is its own component scoped to each portal (intentional duplication: admin and partner pages have different visual chrome, but the modal itself uses the same cream/light treatment as the customer offer modal for visual consistency) |
