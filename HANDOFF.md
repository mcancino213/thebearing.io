...
> Last updated: 2026-05-12 | Current build: v72m
...
| Build | Key changes |
|-------|-------------|
| v72m | **Property index hygiene + slug validation.** Three garbage slugs were showing up in admin-analytics' Live Listing Health: `__index_villas`, `__index_hotels`, `__index_cruises` (curated-order metadata keys that got POSTed as property records in an early version of the reorder feature) plus `singita-grumeti-x` (empty stub record). **Fixes:** (1) `/api/property` GET-all now defensively filters slugs starting with `__`; (2) `/api/property` POST now validates the slug strictly — must be lowercase letters/numbers/hyphens, 2-80 chars, no `__` prefix, no `:`, not a reserved word like `admin/api/settings`. POST also re-cleans the index opportunistically; (3) new endpoint `POST /api/property/cleanup` (admin-gated) — idempotent janitor that scrubs the index, drops `__*` slugs and empty-stub records (no `name` field), reports kept/dropped with reasons; (4) admin-settings.html has a new "Maintenance" section (Section 4) with a "Clean property index" button + JSON output panel; (5) admin-analytics.html double-filters junk slugs client-side as belt-and-suspenders. **Action required:** run the Maintenance → "Clean property index" button once after deploying |
