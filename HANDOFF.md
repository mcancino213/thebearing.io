...
> Last updated: 2026-05-15 | Current build: v73am
...
| Build | Key changes |
|-------|-------------|
| v73am | **Partner email replies now route back to the conversation + portal links land on the right property.** Two bugs Miguel caught after v73al: (1) partner clicks Reply on a `[PARTNER]` notification email \u2014 reply goes to `bookings@thebearing.io` instead of into the conversation. (2) Partner clicks the link in the email \u2014 lands on `pp-conversations.html?id=X` which defaults `?as=` to nour-el-nil and shows an empty list because the conv belongs to a different property. **Fixes:** All 5 outbound partner email paths (new enquiry, guest reply / change request, inbound email reply, offer declined, deposit paid, stale-conversation 24/48/72h reminders) now set `reply_to: reply+{convId}@replies.thebearing.io` so partner replies route to `/api/inbound-email` like customer replies always have. Portal links in those same emails now include `&as={propertySlug}` so the partner-portal page knows which property's conversations to scope to. **Inbound webhook upgraded** to disambiguate sender by from-email: extracts the bare email from `From:` header (handles both `'Name <a@b.com>'` and `'a@b.com'` formats), checks against the conv property's `partner_emails` list (which already includes the transition default when no real emails set), and classifies as `role:'partner'` when matched, falling through to `role:'guest'` otherwise. Notification logic also branches: partner email reply notifies the customer (like a portal-sent partner message would) plus CCs admin for visibility; guest email reply continues to notify admin + partner (existing behavior). **Frontend auto-open:** both `pp-conversations.html` and `admin-conversations.html` now read `?id=X` from URL on load and auto-open that conversation after the list loads. Guarded with a `window._ppAutoOpened` / `_admAutoOpened` flag so it fires once per page load. If the requested conv isn't in the list (e.g. wrong `?as=` slug), a console warning explains why. **Why this matters in practice:** partner can now reply to any notification email directly from Gmail/Outlook and have it appear correctly as a partner message in the conversation thread. The customer sees it as if the property sent it from the portal. No portal login required to reply |
