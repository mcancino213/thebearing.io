...
> Last updated: 2026-05-15 | Current build: v73ao
...
| Build | Key changes |
|-------|-------------|
| v73ao | **Diagnostic instrumentation for the customer unread-badge regression.** Miguel reported after v73an: partner email replies now classify correctly (role:'partner' as expected), but the customer's sidebar Conversations badge still doesn't tick to "1" when a partner replies. Customer was on a different page (not /conversations) at the time, has multiple test convs, and we don't know if KV consistency delay was a factor. Three theories (auto-open-single-conv markRead, KV consistency lag, guestId mismatch) couldn't be confirmed from code audit alone. **Per the project's "stop guessing, gather diagnostic data" rule, this build adds two visibility tools rather than a speculative fix:** (1) **Inbound handler logs** after the unreadGuest bump: `[Inbound] post-bump conv state: convId=X inferredRole=partner guestId=Y unreadGuest=N unreadAdmin=N status=open` and `[Inbound] post-recompute counter: guests[Y]=N admin=N`. These two log lines let us pinpoint exactly which step in the chain fails \u2014 if `unreadGuest=1` but `guests[Y]=0`, recompute is broken; if both show 1 but the customer never sees the badge, the issue is client-side polling or guestId mismatch. (2) **New endpoint `/api/unread-debug`** (admin-gated). Returns the full aggregated counter from KV plus each conversation's individual contribution. Accepts `?guestId=X` to filter to one guest's convs, `?force=recompute` to recompute from scratch first (useful for checking if the stored counter is stale). Example: `https://thebearing.io/api/unread-debug?guestId=user_xxx&force=recompute` shows convs the guest owns, their unread state, and what the freshly-recomputed counter says about them. After deploying, do another partner-reply test, then hit the debug endpoint or check CF logs to figure out where the chain breaks. No behavioral changes \u2014 only logging + read-only diagnostic. |
