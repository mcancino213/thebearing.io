...
> Last updated: 2026-05-13 | Current build: v73a
...
| Build | Key changes |
|-------|-------------|
| v73a | **Three property.html enquiry-form fixes flagged during v72y smoke-test.** Letter rollover from v72z → v73a. (1) **Guest input replaced.** Old form had a single free-text `inq-guests` field with placeholder "2 adults" — but the downstream code in `populateReview()` and `populateConfirmation()` read from `inq-adults` and `inq-children` IDs that didn't exist, silently falling back to defaults of 2/0. Result: whatever the user typed was ignored, and the confirmation screen always showed "2 adults" regardless. Replaced with side-by-side `inq-adults` (type=number, min=1, max=40, default=2) and `inq-children` (type=number, min=0, max=20, default=0). `inputmode="numeric"` for mobile keyboard. (2) **"Open my conversation" button was broken.** Step 2's primary CTA called `submitEnquiryAndGo()` which doesn't exist anywhere in the codebase — clicking it threw a silent ReferenceError and did nothing. Likely a rename leftover from an old refactor. Changed to call `goStep(3)` which advances to the confirmation step (where `populateConfirmation()` actually creates the conversation in KV via `/api/conversation`). (3) **Orphaned HTML debris on Step 3.** Lines after the "Go to conversation" pill link had a duplicate `<svg>` element and an extra `</a>` closing tag — browsers tolerated it but it was wrong. Cleaned up. **All three bugs pre-date v72y** — they were uncovered because the v72z brace fix finally let the enquiry flow render, and the v72y smoke-test exercised the form for the first time in many builds. **Notably: I should have caught the broken "Open my conversation" button at build time** by checking that all `onclick` handlers reference defined functions. Adding to the pre-package validation pass alongside JS syntax check. **Smoke test continuation:** with these fixes, you can now complete the enquiry → offer-builder flow described in v72y's handoff |
| v72z | **Critical fix: property.html had an unclosed `populateConfirmation()` function** that broke the entire page (file line 2156, missing `}`). Bug pre-dated v72y — present in every build going back to at least v72o. Surfaced now because property pages hadn't been visited recently. Single-line surgical fix |
