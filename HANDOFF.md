...
> Last updated: 2026-05-13 | Current build: v73j
...
| Build | Key changes |
|-------|-------------|
| v73j | **Four offer-flow improvements.** (1) **Enquiry message reformatted for readability.** Old format was one run-on sentence with embedded dates. New format: greeting + property/room/pax intro (ending with colon if dates follow), blank line, `Arriving: X / Departing: Y` block, blank line, free-text notes, blank line, closing. Added `white-space:pre-line` to `.ch-bubble` (customer) and `.msg-bubble` (partner+admin) so `\n` characters actually render as line breaks. (2) **"Average Nightly Rate" labels** throughout the partner offer builder — mode-switcher button is "Average nightly × nights", input is "Average nightly rate (USD)" with helper "e.g. nights might vary — this is the blended average", auto-computed package equivalent is "Equivalent average nightly · auto: total ÷ nights". Conveys that variable nightly pricing is normal and the figure is a blend. (3) **Partner can save section defaults per property.** New worker endpoint `/api/property/offer-defaults` (admin-gated POST, allows four whitelisted fields). Three "Save as default" buttons in the offer modal: one each for Inclusions, Cancellation terms, Notes to guest. Each one PATCHes the property record. Next time the partner builds an offer on the same property (and there's no prior offer to revise), the modal prefills those fields from the saved defaults. Buttons turn green ✓ Saved for 2.5s as confirmation. (4) **Offer surfaces on customer side as a clickable card in conversation.** When an offer is sent (POST sendImmediately or PATCH action:'send'), the worker posts an `offer_card` message into the linked conversation. The customer conversation renderer detects `m.type === 'offer_card'` and renders a styled card (cream + terracotta border) with property name, room, dates, total price, deposit amount, validity period, and a "View offer →" CTA. Clicking opens a modal with the full offer breakdown: status badge, header, dates, pricing breakdown, inclusions list (checkmark bullets), exclusions, cancellation terms, partner notes, validity countdown, and Accept-and-pay / Request-changes action buttons. Accept-and-pay shows a placeholder ("activates with Stripe Checkout in the next build") for now; Request-changes closes the modal and pre-fills the conversation composer with "I'd like to discuss a change to this offer:". Partner + admin views see the same offer as a regular text message with the plain-text fallback "Your offer is ready — {property} has prepared a personalised quote." since their views don't need the customer-facing card visualization. **Deferred to v73k:** Stripe Checkout integration for Accept-and-pay flow; the customer `/bookings.html` rewrite with clear Enquiries/Bookings separation |
