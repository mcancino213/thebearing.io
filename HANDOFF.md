...
> Last updated: 2026-05-14 | Current build: v73t
...
| Build | Key changes |
|-------|-------------|
| v73t | **Three v73r/v73s bug fixes caught in Miguel's testing.** (1) **Raw HTML markup leaking through escHtml-wrapped contexts on pp-bookings.** The pendingChangeRequest pill in the bookings row dates column and the "Was/Now" banner in the Revise offer modal both displayed literal `<div style="...">Jun 15 \u2013 Jun 20, 2026</div><div style="..."> ? nights</div>` text instead of formatted dates. Root cause: `fmtDateRange()` returns HTML markup (`<div>` tags with inline styles) intended for direct innerHTML injection. I passed that HTML through `escHtml()` and into a string template, which entity-escaped the tags so the browser rendered them as visible text. Same family of bug as the v73p `\u203a` issue, inverted: there I had text that should have been HTML; here I have HTML that should have been text. Fix: new `fmtDateRangePlain(arrival, departure, nights)` helper that returns plain text ("Jun 15 \u2013 Jun 20, 2026 \u00b7 2 nights"). Both call sites (row pill + modal banner) updated to use the plain variant. The main dates column still uses `fmtDateRange` since it injects directly. (2) **No decline path from the offer modal in conversations/bookings views** \u2014 Miguel correctly flagged the gap from v73r where Decline only existed inside the /property interstitial. If the customer opened the offer directly from `/conversations` or `/bookings`, they had no way to decline. v73r deferred this; v73t lands it. Both modals now show a small "Decline this offer" link below the Request changes + Accept & pay row. Click confirms via window.confirm, then PATCH /api/offer action:'decline' with requesterEmail, then reloads the page so the new system message and updated booking state appear. **Removed stale v73q placeholder copy** ("Accept-and-pay activates with Stripe Checkout in the next build.") from both modals while editing this section since the feature is live now. (3) **2-3 second lag with no visible feedback** on Request changes button click. The cascade (worker write + Resend + page nav + new conversation fetch + render) genuinely takes that long; the perception bug was that the button just dimmed without any indication anything was happening. Fix: replace button content with explicit "Sending your request\u2026" text on click. Original content restored on error. **Cheap perceived-performance fix; doesn't reduce actual latency.** **Auditing improvement:** caught that the `<div>` count audit I'd been running for HTML structural validation was producing false positives from script-block comments and content. New approach for v73t+: strip `<script>` and `<style>` blocks before counting tags. Confirmed pp-bookings, conversations, bookings, property, admin-settings all structurally balanced after this fix |
| v73s | admin-settings layout hotfix: v73r removed an extra `</div>` while restructuring the Maintenance section, leaving the new "Restore booking dates" block floating outside the parent card. Added back one closing `</div>` to repair the structure |
