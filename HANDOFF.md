...
> Last updated: 2026-05-14 | Current build: v73z
...
| Build | Key changes |
|-------|-------------|
| v73z | **Critical hotfix: silent date mutation on already-confirmed bookings.** Miguel discovered that submitting a fresh enquiry as customer on a property where they already had a confirmed booking would (a) overwrite the confirmed booking's dates silently and (b) display the new enquiry as already-Confirmed status without any partner intervention. Two layers of bug. **Layer A (frontend reuse logic, property.html line 2399 + nour-el-nil.html line 2182):** the existing-conversation lookup matched any conversation with `status:'open'` for the same property+guest, then routed the new enquiry as a follow-up message. But a conversation tied to a confirmed booking is still `status:'open'`. So a customer's second separate enquiry got conflated with their first confirmed stay. Fix: filter by linked booking status \u2014 only reuse when the booking is in `enquiry`, `offer_sent`, or `pending` state. Confirmed/cancelled/etc \u2192 create a new conversation + new booking. Frontend now fetches `/api/booking?email=X` in parallel with `/api/conversation?guestId=X` and cross-references via `conv.bookingRef`. **Layer B (worker guard, envoy.js line 1850):** v73r added the `pendingChangeRequest` mechanism but the state check only covered `'offer_sent'` and `'pending'`. Confirmed bookings fell through to the mutation branch, which silently rewrote arrival/departure on the paid booking. New guard is state-aware in three buckets: (a) fresh enquiry no offer ever \u2192 mutate freely; (b) offer_sent/pending \u2192 store pendingChangeRequest; (c) confirmed/cancelled \u2192 do NOTHING, log a warning. Belt-and-suspenders against direct API callers bypassing the frontend filter. **Test corruption alert:** Miguel's test data may have a Nour El Nil X booking with corrupted dates from this bug \u2014 the dates currently shown on that confirmed booking are NOT the dates the original offer was for. Manual repair available via admin-settings "Restore booking dates from active offer" maintenance button (v73r). **Pre-existing tech debt noticed during build:** `nour-el-nil.html` script block 5 fails `node --check` with a JS syntax error inherited from v73t or earlier. Browser tolerates it (page works) but it should be cleaned up in a future build. Not fixed here to avoid scope creep on a hotfix |
