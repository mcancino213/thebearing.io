...
> Last updated: 2026-05-13 | Current build: v73g
...
| Build | Key changes |
|-------|-------------|
| v73g | **Five fixes addressing smoke-test issues from v73f.** (1) **My-account no longer flashes account content for 1-2s after sign-out.** Added `body{visibility:hidden}` CSS rule at top of `<head>` with noscript fallback. The existing JS already flips visibility to `'visible'` after Clerk's auth check resolves (line 953/981) and there's an 8s failsafe. Customer-portal pages without auth gating (bookings, lens, saved, preferences, settings) have the same flash leak in theory but those pages weren't flagged; deferred. (2) **Auth modal centering fixed (third attempt).** v73f's max-width bump (480→560) helped Clerk fit, but on shorter viewports the sheet was taller than the viewport and `align-items:center` was pushing the TOP off-screen. Changed overlay to `align-items:flex-start` + `padding-top:5vh` + `overflow-y:auto` — sheet now starts near the top with breathing room and scrolls within the overlay if the content is taller than viewport. (3) **Sign-in resume from home page.** When Clerk's OAuth flow redirects to the home page (instead of honoring afterSignInUrl — likely because the Clerk dashboard has its own home URL configured), index.html now checks for `tb_pending_action` in sessionStorage on page load. If found and not expired (10-min), redirects to `/property?slug={pending.slug}`. The property.html resume IIFE (added v73f) then picks up the same key and opens the enquiry overlay. So the flow is: Enquire click → sessionStorage set → sign-in modal → OAuth → land on home → index detects pending → bounce to property → property opens overlay. Three hops but seamless. (4) **Enquiry now creates a stub booking record.** Root cause of "no Build offer button in pp-bookings": enquiries were stored ONLY as conversations (`conversation:{id}`), but `/api/booking?slug=X` only returns booking records. The pp-bookings page therefore showed 0 bookings even when conversations existed. Now: `/api/conversation` POST with `action:'create'` also writes a `booking:{ref}` record with `status:'enquiry'`, linked to the conversation via `booking.conversationId` and `conv.bookingRef`. The booking has best-effort first/last name split from guestName, and pulls arrival/departure/guests/room/notes from the enquiry payload. pp-bookings will now show "Enquiry"-status rows with the Build Offer button visible. **Note:** old conversations created pre-v73g do NOT have linked bookings — only NEW enquiries will appear in pp-bookings. To test the offer flow, send a fresh enquiry from a logged-in user. (5) **Partner portal identity is now dynamic.** pp-conversations.html had hardcoded `PP_NAME = 'Nour El Nil'` (used in message attribution at line 945 and senderName at line 1289) and a hardcoded sidebar header (`<div class="sb-context-name">Nour El Nil</div>` etc.). When viewing as `?as=gypsy-by-mekong-kingdoms`, the partner reply was still attributed to "Nour El Nil" and the sidebar still read "River Cruise · Egypt". Now: both pp-conversations and pp-bookings call `fetchPartnerIdentity()` on load, which fetches `/api/property?slug={PP_SLUG}` and updates `PP_NAME`, the sidebar name/role/avatar from the KV record. Role line shows "Type · Country" parsed from `render.location_line` or `details.country`. Initial render is `PP_SLUG` as fallback (replaces hardcoded "Nour El Nil") so any code reading it pre-fetch doesn't see a stale identity. **Deferred:** orphan conversations from earlier builds (created with guestId=email instead of Clerk user.id) still won't show in customer sidebar — separate backfill task |
