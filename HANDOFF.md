...
> Last updated: 2026-05-15 | Current build: v73ai
...
| Build | Key changes |
|-------|-------------|
| v73ai | **Booking record now syncs trip details from offer on send + confirmation.** Bug Miguel caught: customer enquired without specifying dates \u2192 booking record created with `arrival=''`, `departure=''` \u2192 partner sent offer with dates (May 1\u2192May 4) \u2192 customer accepted and paid \u2192 booking record STILL had empty dates on the partner side, showing "Dates TBD" with the Confirmed badge. Root cause: the worker's offer-send path (envoy.js:3233-3242, both POST send-immediately and PATCH action='send') was only writing `offers`, `active_offer_id`, `status`, `updatedAt` to the booking. It never copied the offer's `arrival`, `departure`, `nights`, `guests`, `room` onto the booking. So if the original enquiry was open-ended, the booking stayed dateless forever. Earlier comment in the webhook ("Booking already has arrival/departure/room/guests mutated on offer-send") was simply wrong \u2014 it described intent, not behavior. **Fix:** both offer-send paths now copy the offer's trip details onto the booking when sending (only fills set values, never blanks). Also added defensive backfill in all three payment-confirmation paths (Stripe webhook, customer sync-payment, admin sync-payment) that fills missing trip details from the accepted offer at confirmation time \u2014 covers legacy bookings sent pre-v73ai that may still have empty fields. Both syncs are "only fill empty" \u2014 never overwrite existing booking values, so date corruption from v73r-era bugs isn't reintroduced. **For Miguel's existing stuck Nour El Nil X confirmed booking:** the admin-settings "Sync a Stripe payment to a booking" maintenance button now also backfills trip details, so re-running it on that booking's PI ID would fix the dates. Alternatively, "Restore booking dates from active offer" maintenance does the same thing for any booking with an active offer reference. **v73ah recap:** the View button issue Miguel reported was actually due to v73ah not being deployed yet \u2014 the disabled grey View fallback was rendering from older code. Once v73ah deploys, the View button becomes clickable and opens the detail modal |
