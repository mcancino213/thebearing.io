...
> Last updated: 2026-05-12 | Current build: v72u
...
| Build | Key changes |
|-------|-------------|
| v72u | **Removed misplaced "Property partner? Sign in" link from customer-facing footer.** The dark footer bar on customer pages had a small low-contrast link in the right corner pointing to `pp-login` — out of place on a member-facing page (members shouldn't be invited to sign in as property partners from inside their own account area). Miguel flagged it on lens and saved, but the same block existed on 4 other customer pages too (bookings, preferences, settings, search). All 6 cleaned. The dark footer bar with the © 2026 line stays — only the partner link was removed. The link still exists on the marketing site where it belongs |
