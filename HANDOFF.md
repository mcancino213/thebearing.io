...
> Last updated: 2026-05-13 | Current build: v73c
...
| Build | Key changes |
|-------|-------------|
| v73c | **Four fixes addressing enquiry-flow issues surfaced after v73b.** (1) **Step 3 of enquiry flow eliminated entirely.** Old flow: Step 1 (form) → Step 2 (review) → Step 3 (confirmation + fake conversation preview). The Step 2 button "Open my conversation" said it would open the conversation but instead advanced to a Step 3 that the user then had to dismiss separately. Now: Step 2's button calls `submitEnquiryAndOpenChannel(this)` directly — creates the conversation, navigates to `/conversations.html?id={newId}` immediately. Step 3 panel HTML deleted, third pip dot removed, `goStep` iterates `[1,2]`. (2) **Robust Clerk user-id resolution.** Old code: `var user = tbAuth.getUser(); var guestId = user ? user.id : email;` — if `tbAuth.getUser()` returned null due to a cache lag (Clerk auth still loading), `guestId` fell back to email. Conversation got stored under `guest:email:convs` but `/conversations.html` looks up under `guest:user_xxx:convs` → conversation was invisible in sidebar (but accessible via direct `?id=X` URL since by-id lookup bypasses the guestId index). New code tries `window.Clerk.user` first (authoritative source) → `tbAuth.getUser()` → null. If all return null, refuses to create and shows an inline error: "Please sign in to send your enquiry." This was the root cause of Miguel's "Mekong Kingdoms chat doesn't show up in the sidebar list" bug. (3) **Listing-page enquiry buttons now redirect to property pages.** Pre-v73c the listing pages (cruises/hotels/villas) had a `quick-enquiry-overlay` modal that was pure theatre — `submitQuickEnquiry()` just showed a fake "Enquiry sent" toast and did nothing else (no API call, no conversation created). Now `openQuickEnquiry(propertyName, slug)` redirects to `/property?slug=X` if slug is provided (KV-backed cards), or shows a "Coming soon to The Bearing — we curate properties carefully and this one isn't available to book yet" toast for static demo cards without real KV records. Cruises page: Nour El Nil static card updated with `'nour-el-nil'` slug, KV card builder passes slug. Hotels and Villas pages: all current cards are static demo cards → all show "coming soon" toast (will pass slugs when KV-backed cards land). The obsolete `<div id="quick-enquiry-overlay">` markup is left in place but hidden — cleanup deferred to reduce v73c scope. (4) **Inline error display added.** Submission failures now show `#inq-error` red box on Step 2 instead of silent `console.error`. Network errors, missing slug, missing Clerk user, server-side errors — all surface to the user. Button disables + shows "Opening your conversation…" while in-flight. **Bonus fix:** found and removed a pre-existing double `.catch()` block in villas.html script #2 (lines 1282-1285) — duplicate `.catch()` with a stray `})` between them. Browsers tolerated it but it was wrong; my new per-script `node --check` validator caught it. **Pre-package validation upgraded:** I now validate every inline `<script>` block in modified files individually via Python+Node, not just specific known scripts. That's how the villas bug was caught |
