...
> Last updated: 2026-05-12 | Current build: v72p
...
| Build | Key changes |
|-------|-------------|
| v72p | **Conversation scroll-to-bottom fix across admin / customer / partner portals.** Bug: opening a conversation showed the second-to-last message at the bottom of the pane while the actual last message hid behind the composer. **Root cause:** the existing `setTimeout(scroll, 50)` pattern fires before layout fully settles — avatar images, emoji reactions, web fonts, and the composer transition all reflow *after* the initial scroll, pushing content down while the scroll position stays at "the old scrollHeight". **Fix:** new shared helper `assets/conv-scroll.js` exposes `window.pinToBottom(wrap, opts)` which (1) uses `scrollIntoView({block:'end'})` on the last child instead of brute scrollTop (browser handles composer padding correctly), (2) repeats across 3 animation frames to catch late reflows, and (3) attaches a 1-second ResizeObserver on the wrap and its last child so any image-load / reaction-render reflow re-pins to bottom. Applied at all 6 sites: open-conversation in admin-conversations.html / conversations.html / pp-conversations.html, plus the polling-refresh path on each (refresh uses `{ observe: false }` since layout is already settled). The four `ci-messages` scrolls in conversations.html are the Envoy AI chatbot drawer — different system, intentionally left alone |
