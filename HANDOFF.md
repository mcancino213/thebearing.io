...
> Last updated: 2026-05-12 | Current build: v72w
...
| Build | Key changes |
|-------|-------------|
| v72w | **Stripe SDK scaffolding — connection ready, no flow yet.** This build wires Stripe into the worker but adds no member-facing payment flow. **Worker changes:** (1) New `package.json` with `stripe: ^17.0.0` dependency — first npm dep in the project; Cloudflare Pages installs at build time. (2) `import Stripe from 'stripe'` at top of `functions/api/envoy.js`. (3) Lazy `getStripe(env)` helper that constructs a module-cached Stripe client using `Stripe.createFetchHttpClient()` — the default SDK HTTP client uses `node:https` which doesn't exist on Workers, so without this the SDK throws on first call. (4) New `GET /api/stripe/health` (admin-gated) — calls `accounts.retrieve()` and returns `{configured, ok, mode, account_id, country, ...}`. Returns `{configured:false}` if no key set, `{configured:true, ok:false, error}` if Stripe rejects the key. (5) New `POST /api/stripe/webhook` (public, signature-verified) — uses `constructEventAsync` (NOT `constructEvent`; the sync version uses Node crypto APIs that fail silently on Workers — well-known Cloudflare/Stripe footgun). For v72w it just logs and acknowledges; real branching on `event.type` arrives with v72z. (6) `/api/health` extended with a `stripe` check alongside KV/Resend/etc. **Admin changes:** (a) `admin-settings` System Health card has a new Stripe row showing mode (test/live), account ID, and a note about webhook secret. (b) `admin-payments` status banner now probes `/api/stripe/health` on load and flips state: still-pending shows the v72v amber banner; connected (test) shows a green-tinted "Stripe connected · test mode" with account ID; auth error shows a red banner with the Stripe error message. **Required after deploying:** Miguel must add two encrypted env vars in Cloudflare Pages dashboard (production environment): `STRIPE_PUBLISHABLE_KEY` and `STRIPE_SECRET_KEY` (test keys, `pk_test_*` / `sk_test_*`). `STRIPE_WEBHOOK_SECRET` (`whsec_*`) is added later — needed by v72z, not v72w; the webhook endpoint will respond 503 until set. **Out of scope:** no checkout, no offer flow, no commission_pct field, no booking flow changes. All deferred to v72x→v72z |
