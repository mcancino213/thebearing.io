...
> Last updated: 2026-05-14 | Current build: v73aa
...
| Build | Key changes |
|-------|-------------|
| v73aa | **Five must-do polish items.** (1) **Broken Enquire button on scrolling book bar fixed + Book now hidden.** Root cause: the `.book-bar-enq-btn` class only had styling inside a mobile media query \u2014 on desktop the button rendered unstyled (white box with browser default border). Bundled into the consolidation request: removed Book now entirely from the book bar AND desktop sticky card (since instant-book isn't built yet), replaced with single prominent terracotta "Enquire to book" CTA. Applied across both property.html and nour-el-nil.html. (2) **Conversation list shows enquiry dates** for disambiguating multiple convs on the same property. Customers who have one confirmed booking and one new enquiry at Nour El Nil X (v73z scenario) now see dates as a subline below the property name in the list \u2014 e.g. "Nour El Nil X" then "1 Sep \u2192 8 Sep 2026". Added in `chBuildConvItem`. Falls back to no subline when conv.enquiry.arrival/departure absent. (3) **Customer sidebar bookings badge for new enquiries.** Discovered a pre-existing bug while building: `customer-sidebar-badges.js` was hitting `/api/booking?guestId=X`, but that endpoint only filters by `email`, so the guestId param was silently ignored and the badge was counting bookings across ALL guests in the system. Fixed to use `?email={primaryEmail}` and also tightened the count to only items needing customer action (offers awaiting response, new enquiries awaiting partner) \u2014 confirmed and cancelled bookings excluded. (4) **Expandable confirmed booking details.** Confirmed booking rows on /bookings are now clickable, expanding inline to a rich detail panel: total stay, deposit paid (with date), balance due (with date or "due directly with property" if no date set), inclusions, exclusions, cancellation terms, partner notes, link to conversation thread. Click row again to collapse. Chevron icon rotates on expand. (5) **Balance due date added to offer schema.** New `balance_due_date` field on offer record + new `<input type="date" id="om-balance-due">` in the partner offer builder modal (next to "Offer valid until"). Optional \u2014 empty means "due directly with property at time of stay" (current default for cruises/all-inclusives). Snapshotted to the booking record at payment confirmation alongside the other offer details so the customer detail view can read everything from the booking without an extra offer fetch. **Older bookings confirmed before v73aa won't have the snapshot fields** \u2014 the detail panel shows a "Some details unavailable" notice for those. To backfill, would need a one-time maintenance script reading each confirmed booking's offer record |
