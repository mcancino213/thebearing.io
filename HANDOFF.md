...
> Last updated: 2026-05-12 | Current build: v72y
...
| Build | Key changes |
|-------|-------------|
| v72y | **Offer model + partner offer builder + pp-bookings rewritten with real data.** Largest build in the payment sequence. **Worker — new `/api/offer` endpoint family:** (1) `GET ?id=X` fetches one offer. (2) `GET ?bookingId=X` lists all offers for a booking (oldest first). (3) `POST` creates an offer — required: `bookingId`, `propertySlug`, `total_amount`. Snapshots the property's `commission_pct` into `commission_pct_at_time` (frozen — later changes to the property don't retroactively alter the offer). Computes `deposit_amount = total × commission_pct / 100`. Defaults valid_until to +7 days. Auto-generates `OFR-{year}-{rand}` ID. **Refuses creation if property has `commission_pct === null`** — this is where v72x's nullable field becomes truly required. (4) `PATCH` supports three actions: `send` (draft → sent), `withdraw` (draft/sent → withdrawn), `edit-draft` (only allowed when status === 'draft'). Member-driven transitions (accept/decline/changes_requested) deferred to v72z. **Offer state machine:** draft → {sent, withdrawn}; sent → {accepted, declined, changes_requested, superseded, expired, withdrawn}. Revisions: new offer with `supersedes: oldOfferId` automatically flips the old offer to `superseded` status and links both directions. **Worker — `/api/booking` extended:** (a) `?slug=X` filter added so partner portal can scope to its own bookings; (b) on-read backfill of `offers: []` and `active_offer_id: null` on legacy booking records (idempotent). **New KV keys:** `offer:{id}` for offer records, `__offers_by_booking:{ref}` for per-booking offer history. Booking records gain `offers: [ids]` and `active_offer_id`. New booking status value: `offer_sent`. **`pp-bookings.html` — complete rewrite:** Dummy data block (Sarah Alderton / Thomas Müller / fake $82,600 KPIs) deleted. Page now fetches `/api/booking?slug=nour-el-nil` on load, renders real KPIs (open enquiries / offers sent / confirmed / avg lead time), table with status badges + state-aware action buttons. Filter tabs (All / Enquiries / Offers sent / Confirmed) + client-side search across guest/email/ref. **Offer builder modal:** right-anchored sheet with structured form — arrival/departure (auto-computes nights), guests, room/package name, pricing mode toggle (Package total OR Per-night × nights), USD total, repeatable inclusion items (one per row, partner adds as many as needed), exclusions free text, cancellation terms (required), notes to guest, validity date (default +7d). Live deposit preview card showing `15% of $5,000 = $750` math. Two submit actions: "Save as draft" or "Send offer to guest →". When revising an existing sent offer, modal pre-fills from previous offer and submits with `supersedes` linkage. **Important deferred work:** Partner portal still has NO real authentication — `PP_SLUG` is hardcoded to `'nour-el-nil'` across all pp-*.html files. Any visitor can read/write Nour El Nil's bookings and create offers against it. Acceptable for Stripe test mode and smoke-testing v72y/v72z, but MUST be addressed before going live. Multi-tenant Clerk partner auth is on the deferred-work list in PROJECT_CONTEXT.md |
